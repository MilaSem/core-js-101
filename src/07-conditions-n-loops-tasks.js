...
function isTriangle(a, b, c) {
  return a > Math.abs(b - c) && b > Math.abs(c - a) && c > Math.abs(a - b);
}
...
function isInsideCircle(circle, point) {
  const cat1 = circle.center.x - point.x;
  const cat2 = circle.center.y - point.y;
  const hip = Math.sqrt(cat1 ** 2 + cat2 ** 2);

  return hip < circle.radius;
}
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  const char1 = isStartIncluded === true ? '[' : '(';
  const char4 = isEndIncluded === true ? ']' : ')';
  const [char2, char3] = a < b ? [a, b] : [b, a];
